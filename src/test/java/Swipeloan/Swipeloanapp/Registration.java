...
//		driver.manage().timeouts().implicitlyWait(30, TimeUnit.SECONDS);
		...
		Thread.sleep(1000);	
		  ...
		  WebElement otpInputField = driver.findElement(By.className("android.widget.EditText"));
		    ...
		    // Option 2: If the app requires entering OTP digits into separate fields:
		  ...
		  //      otpFields.get(0).clear();
		    ...
		    }/* else if (otpFields.size() >= otp.length()) {
		        // Multiple fields case
		        System.out.println("Multiple OTP input fields detected.");
		        char[] otpDigits = otp.toCharArray();
		        for (int i = 0; i < otpDigits.length; i++) {
		            WebElement field = otpFields.get(i);
		            wait.until(ExpectedConditions.elementToBeClickable(field)); // Ensure field is clickable
		            field.click();
		            field.clear();
		            field.sendKeys(String.valueOf(otpDigits[i]));
		            Thread.sleep(500); // Slight delay to handle auto-focus
		        }  */
...
*/
     	...
     	WebElement panEditText = wait2.until(ExpectedConditions.elementToBeClickable(
     			By.xpath("//*[@hint='Enter PAN Number']")
     	...
     	driver.hideKeyboard();
         Thread.sleep(5000);
         WebElement continueButton = wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//android.widget.Button[@content-desc='Verify']")));
